...
double permanentFastest(const Eigen::MatrixXd& A) {
	...
	if ((A.rows() <= 3 && A.cols() <= 50) || (A.rows() <= 5 && A.cols() <= 20)) {
		return permanentExactPlain(A);
	}
	else {
		if (this_dim <= switch_dimension) {
			return permanentExact(A);
		} else {
			// std::cout << "use approximate permanent!" << std::endl;
			return permanentApproximation(A, 300);
		}
	}
...
double permanentExactSquare(const Eigen::MatrixXd& A) {
  ...
  {	
	std::cout << "Matrix dimension: " << m << std::endl;
...
// double permanentExactPlain(const Eigen::MatrixXd& A)
// {
// 	int K = A.rows();
//     int N = A.cols(); 
//     double perma = 0;
...
//             double prod = 1;
//             for (int j = 0; j < K; ++j) {
//                 // std::cout << selected_cols[j] << " ";
//                 prod *= A(j, selected_cols[j]);
//             }
//             perma += prod;
//             // std::cout << std::endl;
//         } while (std::next_permutation(selected_cols.begin(), selected_cols.end()));
...
double permanentExactPlain(const Eigen::MatrixXd& A) {
	...
	if (m == 1) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                if (i == j) continue;
                double prodj = prodi * A(1, j);
                for (int k = 0; k < n; ++k) {
                    if (k == i || k == j) continue;
                    // perma += A(0, i) * A(1, j) * A(2, k);
                    perma += prodj * A(2, k);
                }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                for (int k = 0; k < n; ++k) {
                    if (k == i || k == j) continue;
                    double prodk = prodj * A(2, k);

                    for (int l = 0; l < n; ++l) {
                        if (l == i || l == j || l == k) continue;
                        double prodl = prodk * A(3, l);

                        for (int p = 0; p < n; ++ p) {
                            if (p == i || p == j || p == k || p == l) continue;
                            // perma += A(0, i) * A(1, j) * A(2, k) * A(3, l) * A(4, p);
                            perma += prodl * A(4, p);
                        }   
                    }   
                }
